...
void dmdNombreOperateur(int *nombreOperateur) {
    if (*nombreOperateur == -1) {
        printf("Combien d'opérateurs sont disponibles aujourd'hui ? \n");
        scanf("%d", nombreOperateur);
        printf("%d opérateurs seront disponibles aujourd'hui\n", *nombreOperateur);
    }
}
